"""Auto-generated strategy: Leech-on-Whale Directional Alignment Strategy
  ID: C028 | Tier: tier1 | Source: composed
  Components: entry: 1, exit: 1, filter: 1, market_structure: 1 | Chunks: 4
"""
import numpy as np
import pandas as pd
...
class GenStrategy_C028(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1h"
    can_short = False
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Entry: vwap_bounce
        long_conditions = (dataframe["close"] < dataframe["vwap"]) & (dataframe["vwap_dist"] > -0.02) & qtpylib.crossed_above(dataframe["close"], dataframe["vwap"])

        # Short:
        short_conditions = (dataframe["close"] > dataframe["vwap"]) & (dataframe["vwap_dist"] < 0.02) & qtpylib.crossed_below(dataframe["close"], dataframe["vwap"])
        ...
        dataframe.loc[long_conditions, ["enter_long", "enter_tag"]] = [1, "C028_long"]
        dataframe.loc[short_conditions, ["enter_short", "enter_tag"]] = [1, "C028_short"]
        return dataframe
