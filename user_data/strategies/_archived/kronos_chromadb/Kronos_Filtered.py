"""
Kronos+ChromaDB — Session & Regime Filtered.

Base: VectorStrategy_P3E_KEY_LEVEL_BOOST architecture.
Adds: ChromaDB session filters (kill zones) + volatility regime filter.
"""
...
from kronos_indicators import add_session_filters, add_regime_filter


class Kronos_Filtered(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1h"
    ...
    # Filter: allow trading in low-probability sessions?
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ── Regime filter (avoid high volatility entries) ──
        regime_ok = (dataframe["regime_high_vol"] == 0)

        # ── Volume filter (avoid anemic volume) ──
        vol_ok = (dataframe["volume_anemic"] == 0)

        # Combine all filters
        filters_ok = session_ok & regime_ok & vol_ok & (dataframe["volume"] > 0)

        # ── LONG SIGNALS (same as P3E_KEY_LEVEL_BOOST with filters) ──
