"""
VectorOmni_CandleFilter — Candle Pattern + Session Filtered Vector Strategy

Manipulates VectorStrategy_P3E_KEY_LEVEL_BOOST by adding:
  1. 13 TA-Lib candlestick patterns as entry pre-filter (from kronos_indicators)
  2. Session kill-zone filters (Asia/London/NY overlap from ChromaDB)
  3. Candle confluence scoring as additional signal pillar
  4. Bullish/bearish pattern confirmation gates all entries

Concept: "Only enter when candle pattern confirms direction" (Kronos engine)
"""
...
class VectorOmni_CandleFilter(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1h"
    can_short = False
    ...
    min_candle_score = IntParameter(0, 3, default=1, space="buy")
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # ── TA-Lib Candlestick Patterns (13 patterns) ──
        dataframe["cdl_doji"] = ta.CDLDOJI(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_hammer"] = ta.CDLHAMMER(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_shooting_star"] = ta.CDLSHOOTINGSTAR(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_marubozu"] = ta.CDLMARUBOZU(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_engulfing"] = ta.CDLENGULFING(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_harami"] = ta.CDLHARAMI(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_piercing"] = ta.CDLPIERCING(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_dark_cloud"] = ta.CDLDARKCLOUDCOVER(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_morning_star"] = ta.CDLMORNINGSTAR(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"], penetration=0.3)
        dataframe["cdl_evening_star"] = ta.CDLEVENINGSTAR(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"], penetration=0.3)
        dataframe["cdl_3_white_soldiers"] = ta.CDL3WHITESOLDIERS(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])
        dataframe["cdl_3_black_crows"] = ta.CDL3BLACKCROWS(dataframe["open"], dataframe["high"], dataframe["low"], dataframe["close"])

        # Bullish score: sum of bullish patterns
        ...
        dataframe["candle_signal"] = dataframe["candle_bullish"] - dataframe["candle_bearish"]

        # ── Session Filters (Kill Zones) ──
        hour = dataframe["date"].dt.hour
        dataframe["session_london_open"] = ((hour >= 7) & (hour < 9)).astype(int)
        dataframe["session_ny_overlap"] = ((hour >= 13) & (hour < 17)).astype(int)
        dataframe["session_kill_zone"] = (dataframe["session_london_open"] | dataframe["session_ny_overlap"]).astype(int)
        dataframe["session_asia"] = ((hour >= 0) & (hour < 8)).astype(int)

        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        candle_ok_long = (dataframe["candle_signal"] >= self.min_candle_score.value)
        candle_ok_short = (-dataframe["candle_signal"] >= self.min_candle_score.value)
        ...
        dataframe.loc[
            ...
            candle_ok_long &
            (dataframe["session_kill_zone"] == 1) &
            (dataframe["volume"] > 0),
            ["enter_long", "enter_tag"]
        ] = (1, "candle_filter_long")
