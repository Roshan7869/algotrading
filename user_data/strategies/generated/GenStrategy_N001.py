"""Auto-generated strategy: Complete Technical Analysis Guide
  ID: N001 | Tier: tier1 | Source: natural_group
  Components: entry: 6, filter: 2, risk_management: 1, market_structure: 1 | Chunks: 10
"""
import numpy as np
import pandas as pd
...
class GenStrategy_N001(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1h"
    can_short = False

    minimal_roi = {"0": 0.12, "60": 0.06, "240": 0.03, "720": 0.01}
    stoploss = -0.08
    trailing_stop = True
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["bullish_engulf"] = (dataframe["close"] > dataframe["open"]) & (dataframe["close"].shift(1) < dataframe["open"].shift(1)) & (dataframe["close"] > dataframe["open"].shift(1)) & (dataframe["open"] < dataframe["close"].shift(1))
        ...
        dataframe["swing_high"] = (dataframe["high"] == dataframe["high"].rolling(window=20, center=True).max()).astype(int)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Entry: engulfing
        long_conditions = dataframe["bullish_engulf"] & (dataframe["volume"] > dataframe["volume_ma"])

        # Short:
        short_conditions = dataframe["bearish_engulf"] & (dataframe["volume"] > dataframe["volume_ma"])
        ...
        dataframe.loc[long_conditions, ["enter_long", "enter_tag"]] = [1, "N001_long"]
        dataframe.loc[short_conditions, ["enter_short", "enter_tag"]] = [1, "N001_short"]
        return dataframe
