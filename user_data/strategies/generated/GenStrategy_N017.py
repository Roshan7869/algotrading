"""Auto-generated strategy: Order Flow Trader Reveals Exact System
  ID: N017 | Tier: tier1 | Source: natural_group
  Components: entry: 2, filter: 2, risk_management: 1, market_structure: 4, trade_management: 1 | Chunks: 10
"""
import numpy as np
import pandas as pd
...
class GenStrategy_N017(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "1h"
    can_short = False

    minimal_roi = {"0": 0.02, "15": 0.01, "30": 0}
    stoploss = -0.025
    trailing_stop = True
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Entry: pullback
        long_conditions = (dataframe["close"] < dataframe["ema_medium"]) & (dataframe["close"] > dataframe["ema_trend"]) & (dataframe["adx"] > 20)

        # Short:
        short_conditions = (dataframe["close"] > dataframe["ema_medium"]) & (dataframe["close"] < dataframe["ema_trend"]) & (dataframe["adx"] > 20)
        ...
        dataframe.loc[long_conditions, ["enter_long", "enter_tag"]] = [1, "N017_long"]
        dataframe.loc[short_conditions, ["enter_short", "enter_tag"]] = [1, "N017_short"]
        return dataframe
