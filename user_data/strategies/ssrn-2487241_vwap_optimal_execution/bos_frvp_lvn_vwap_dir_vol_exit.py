...
=========================================================
COMBINED: Paper 1 (VWAP Directional Filter) + Paper 3 (Volume-Weighted Exit)
- Entry: VWAP directional filter (long above VWAP, short below VWAP)
- Exit: Volume-weighted trailing stop (tighten when volume < 85% of 24h avg)
...
Exit: Structure-based (BOS reversal) + RSI exhaustion + volume-weighted trailing
"""
...
class BOS_FRVP_LVN_VWAP_DirVolExit(IStrategy):
    INTERFACE_VERSION = 3
    can_short = True
    timeframe = "1h"
    ...
    def _compute_confluence(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ── SHORT (VWAP directional: short only below VWAP) ──
        ...
        # ── LONG (VWAP directional: long only above VWAP) ──
