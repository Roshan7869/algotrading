"""
V5: BOS+LVN+VWAP — Hyperopt-Ready + VWAP Directional Filter
=============================================================
VWAP directional filter: long only above VWAP, short only below VWAP.
...
class BOS_V5_Hyperopt_Dir(IStrategy):
    ...
    def _compute_confluence(self, df):
        ...
        df["price_below_vwap"] = (df["close"] < df["vwap"]).astype(int)

        # SHORT pillars
        df["short_bos"] = (df["bos_trend"] == -1).astype(int)
        df["short_lvn"] = df["price_in_lvn"]
        df["short_vwap_relaxed"] = (df["near_vwap"] & df["price_below_vwap"]).astype(int)
        ...
        df["long_vwap_relaxed"] = (df["near_vwap"] & df["price_above_vwap"]).astype(int)
