...
VWAP Directional Filter: long only when price ABOVE VWAP, short only when price BELOW VWAP.
...
class BOS_FRVP_LVN_VWAP_Dir(IStrategy):
    INTERFACE_VERSION = 3
    can_short = True
    timeframe = "1h"
    ...
    def _compute_confluence(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # VWAP directional: short only when price is BELOW VWAP
        ...
        # VWAP directional: long only when price is ABOVE VWAP
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        ] = (1, "bos_frvp_lvn_vwap_dir_long")

        dataframe.loc[
            short_cond & (dataframe["volume"] > 0),
            ["enter_short", "enter_tag"]
        ] = (1, "bos_frvp_lvn_vwap_dir_short")
