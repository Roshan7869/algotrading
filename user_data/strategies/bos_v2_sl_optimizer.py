"""
V2: BOS+LVN+VWAP SHORT — Stop Loss Optimizer (4%, 6%, 8%)
============================================================
Three strategy classes with identical entry/exit logic but different
hard stops to find the sweet spot.

From the study: 8% stop had 241/435 trades hit stop at 0% WR.
Tighter stops should reduce DD but may cut winning trades short.
"""
...
def _populate_indicators_base(self, dataframe, metadata):
    """Shared indicator computation for all V2 variants."""
    df = dataframe.copy()
    delta = df["close"].diff()
...
def _entry_exit(self, dataframe, metadata):
    dataframe.loc[
...
def _exit(self, dataframe, metadata):
    dataframe.loc[
...
class BOS_V2_Short_SL4(IStrategy):
    """4% hard stop — tighter, fewer losers, may cut some winners"""
    INTERFACE_VERSION = 3
    can_short = True
    timeframe = "1h"
    ...
    populate_indicators = _populate_indicators_base
    _compute_vwap = _compute_vwap
    _compute_bos = _compute_bos
    _compute_volume_profile = _compute_volume_profile
    _compute_confluence = _compute_confluence
    populate_entry_trend = _entry_exit
    populate_exit_trend = _exit
...
class BOS_V2_Short_SL6(IStrategy):
    """6% hard stop — middle ground"""
    INTERFACE_VERSION = 3
    can_short = True
    timeframe = "1h"
    startup_candle_count = 100
...
class BOS_V2_Short_SL8(IStrategy):
    """8% hard stop — baseline from original study"""
    INTERFACE_VERSION = 3
    can_short = True
    timeframe = "1h"
    startup_candle_count = 100
