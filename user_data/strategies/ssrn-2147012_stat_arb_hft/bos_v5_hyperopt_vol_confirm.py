"""
V5: BOS+LVN+VWAP — Hyperopt-Ready + Volume Confirmation
==========================================================
Paper 2 (ssrn-2147012) volume confirmation: only enter when volume > 24h avg.
All key parameters enabled for optimization:
 - swing_lookback: 10-40 (swing detection window)
 - lvn_threshold: 0.2-0.7 (low volume node threshold)
 - vwap_proximity_pct: 0.3-1.5 (% distance from VWAP)
 - min_body_ratio: 0.3-0.7 (confirmation candle threshold)
 - stoploss: 3%-10% (hard stop)
 - trailing_stop_positive: 1-4% (trail distance)
 - trailing_stop_positive_offset: 5-25% (trail activation)
 - min_confluences: 3 or 4

Run: freqtrade hyperopt --strategy BOS_V5_Hyperopt --hyperopt-loss SharpeHyperOptLoss \
     --spaces buy sell --timeframe 1h --timerange 20260501-20260518 \
     -e 500 --config strat_optimisation/configs/config_v5_hyperopt.json
"""
...
class BOS_V5_Hyperopt_VolConfirm(IStrategy):
    """BOS+LVN+VWAP with all parameters enabled for hyperopt"""
    ...
    stoploss = -0.08  # Will be overridden by hyperopt via sell space
    minimal_roi = {"0": 100}
    ...
    swing_lookback = IntParameter(10, 40, default=20, space="buy", optimize=True)
    volume_bins = IntParameter(10, 40, default=25, space="buy", optimize=True)
    lvn_threshold = DecimalParameter(0.2, 0.7, default=0.5, decimals=2, space="buy", optimize=True)
    vwap_proximity_pct = DecimalParameter(0.3, 1.5, default=0.5, decimals=2, space="buy", optimize=True)
    min_body_ratio = DecimalParameter(0.3, 0.7, default=0.4, decimals=2, space="buy", optimize=True)
    min_confluences = IntParameter(3, 4, default=3, space="buy", optimize=True)

    # Stop loss optimization
    sl_pct = DecimalParameter(0.03, 0.10, default=0.08, decimals=2, space="sell", optimize=True)

    # Trailing stop optimization
    trail_pct = DecimalParameter(0.01, 0.05, default=0.025, decimals=3, space="sell", optimize=True)
    trail_offset_pct = DecimalParameter(0.05, 0.25, default=0.12, decimals=2, space="sell", optimize=True)

    leverage_num = DecimalParameter(5, 15, default=12.0, decimals=0, space="buy", optimize=False)
    ...
    def populate_indicators(self, dataframe, metadata):
        ...
        df = self._compute_volume_confirm(df)
    ...
    def _compute_volume_confirm(self, df):
        df["volume_ma24"] = df["volume"].rolling(24).mean()
        df["volume_confirm_long"] = (df["volume"] * 1.1 > df["volume_ma24"]).astype(int)
        df["volume_confirm_short"] = (df["volume"] * 1.1 > df["volume_ma24"]).astype(int)
        return df
    ...
    def _compute_confluence(self, df):
        ...
        # Confluence thresholds based on min_confluences parameter
        if min_conf >= 4:
            df["short_entry"] = ((df["short_bos"] == 1) & (df["short_pillars"] >= 4)).astype(int)
            df["long_entry"] = ((df["long_bos"] == 1) & (df["long_pillars"] >= 4)).astype(int)
        else:
            df["short_entry"] = ((df["short_bos"] == 1) & (df["short_pillars"] >= 3)).astype(int)
            df["long_entry"] = ((df["long_bos"] == 1) & (df["long_pillars"] >= 3)).astype(int)
    ...
    def populate_entry_trend(self, dataframe, metadata):
        dataframe.loc[
            (dataframe["short_entry"] == 1) &
            (dataframe["rsi_not_oversold"] == 1) &
            (dataframe["volume_confirm_short"] == 1) &
            (dataframe["volume"] > 0),
            ["enter_short", "enter_tag"]
        ] = (1, "bos_short_hyperopt_vol")

        dataframe.loc[
            (dataframe["long_entry"] == 1) &
            (dataframe["rsi"] < 70) &
            (dataframe["volume_confirm_long"] == 1) &
            (dataframe["volume"] > 0),
            ["enter_long", "enter_tag"]
        ] = (1, "bos_long_hyperopt_vol")
    ...
    # Hyperopt will tune these via sl_pct, trail_pct, trail_offset_pct
