...
Paper 2 (ssrn-2147012) Volume Confirmation: only enter when current
volume > 24h average (HFT-driven volume = more reliable BOS signals).
...
class BOS_FRVP_LVN_VWAP_VolConfirm(IStrategy):
    INTERFACE_VERSION = 3
    can_short = True
    timeframe = "1h"
    ...
    def populate_indicators(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # Volume confirmation (Paper 2: HFT-driven volume = reliable BOS)
    ...
    def _compute_volume_confirm(self, df: pd.DataFrame) -> pd.DataFrame:
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe.loc[
            long_cond & (dataframe["volume_confirm_long"] == 1) & (dataframe["volume"] > 0),
            ["enter_long", "enter_tag"]
        ] = (1, "bos_frvp_lvn_vwap_vol_long")

        dataframe.loc[
            short_cond & (dataframe["volume_confirm_short"] == 1) & (dataframe["volume"] > 0),
            ["enter_short", "enter_tag"]
        ] = (1, "bos_frvp_lvn_vwap_vol_short")
