...
=========================================================
CVD-Enhanced: uses real taker buy/sell volume data from Binance
aggTrades for entry confirmation instead of the simplistic
volume > 24h average filter.

Entry: only long when CVD > 0 (taker buying), only short when CVD < 0 (taker selling)
Exit: CVD divergence - exit long when CVD turns negative while price still rising
"""
...
class BOS_FRVP_LVN_VWAP_CVD(IStrategy):
    INTERFACE_VERSION = 3
    can_short = True
    timeframe = "1h"
    ...
    min_buy_ratio = DecimalParameter(0.48, 0.55, default=0.51, decimals=2, space="buy", optimize=False)
    ...
    def _load_cvd(self, df: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        pair = metadata.get("pair", "")
        symbol = pair.split("/")[0] + "USDT"
        cvd_path = Path(f"user_data/data/binance/cvd/{symbol}_cvd_1h.feather")
        ...
        if cvd_path.exists():
            try:
                cvd = pd.read_feather(cvd_path)
                cvd["date"] = pd.to_datetime(cvd["date"], utc=True)
                cvd = cvd.rename(columns={
                    "cvd": "cvd_val",
                    "buy_ratio": "buy_ratio_val",
                    "cvd_divergence": "cvd_divergence_val",
                })

                df["date"] = pd.to_datetime(df["date"], utc=True)
                df = df.merge(
                    cvd[["date", "cvd_val", "buy_ratio_val", "cvd_divergence_val"]],
                    on="date", how="left"
                )
                df["cvd"] = df["cvd_val"].fillna(0)
                df["buy_ratio"] = df["buy_ratio_val"].fillna(0.5)
                df["cvd_divergence"] = df["cvd_divergence_val"].fillna(0)
                df["cvd_positive"] = (df["cvd"] > 0).astype(int)
                df["cvd_has_data"] = df["cvd_val"].notna().astype(int)
                df.drop(columns=["cvd_val", "buy_ratio_val", "cvd_divergence_val"], inplace=True)
            except Exception:
                pass

        min_br = float(self.min_buy_ratio.value)
        df["cvd_confirm_long"] = ((df["cvd"] > 0) & (df["buy_ratio"] >= min_br) & (df["cvd_has_data"] == 1)).astype(int)
        df["cvd_confirm_short"] = ((df["cvd"] < 0) & (df["buy_ratio"] <= (1 - min_br)) & (df["cvd_has_data"] == 1)).astype(int)
        df["cvd_confirm_fallback"] = (df["cvd_has_data"] == 0).astype(int)

        df["volume_ma24"] = df["volume"].rolling(24).mean()
        df["volume_high"] = (df["volume"] * 1.1 > df["volume_ma24"]).astype(int)

        df["cvd_final_long"] = (
            (df["cvd_confirm_long"] == 1) |
            ((df["cvd_confirm_fallback"] == 1) & (df["volume_high"] == 1))
        ).astype(int)
        df["cvd_final_short"] = (
            (df["cvd_confirm_short"] == 1) |
            ((df["cvd_confirm_fallback"] == 1) & (df["volume_high"] == 1))
        ).astype(int)
    ...
    def _compute_confluence(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["short_full_confluence"] = (
            (df["short_bos"] == 1) & (df["short_pillars"] >= 4)
        ).astype(int)
        df["short_partial_confluence"] = (
            (df["short_bos"] == 1) & (df["short_pillars"] >= 3)
        ).astype(int)
        ...
        df["long_full_confluence"] = (
            (df["long_bos"] == 1) & (df["long_pillars"] >= 4)
        ).astype(int)
        df["long_partial_confluence"] = (
            (df["long_bos"] == 1) & (df["long_pillars"] >= 3)
        ).astype(int)
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        dataframe.loc[
            long_cond & (dataframe["cvd_final_long"] == 1) & (dataframe["volume"] > 0),
            ["enter_long", "enter_tag"]
        ] = (1, "bos_cvd_long")

        dataframe.loc[
            short_cond & (dataframe["cvd_final_short"] == 1) & (dataframe["volume"] > 0),
    ...
    def custom_exit(self, pair, trade, current_time, current_rate, current_profit, **kwargs):
        ...
        if current_profit > 0.02:
            cvd = last.get("cvd", 0)
            if not trade.is_short and cvd < -100000:
                return "cvd_divergence_exit_long"
            if trade.is_short and cvd > 100000:
                return "cvd_divergence_exit_short"
