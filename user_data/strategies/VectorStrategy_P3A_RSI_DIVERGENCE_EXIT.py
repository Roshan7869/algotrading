"""
ChromaDB Vector Strategy — P3A: RSI Divergence Exit
=====================================================
P2★ baseline + RSI divergence exit detection in custom_exit().

Enhancement:
  - RSI bearish divergence for longs: price makes higher high, RSI makes lower high
  - RSI bullish divergence for shorts: price makes lower low, RSI makes higher low
  - ChromaDB score 0.464, "most reliable exit across all conditions"
  - 2-candle confirmation via lookback over completed candles only
...
class VectorStrategy_P3A_RSI_DIVERGENCE_EXIT(IStrategy):
    INTERFACE_VERSION = 3
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: datetime,
        ...
        # ── P3A: RSI Divergence Exit ────────────────────────────────
        div = self._detect_rsi_divergence(dataframe)
        if trade.is_short and div["bullish"]:
            return "rsi_bullish_divergence"
        if not trade.is_short and div["bearish"]:
            return "rsi_bearish_divergence"

        return None

    # ── P3A: RSI Divergence Detection ──────────────────────────────
    def _detect_rsi_divergence(self, dataframe: DataFrame, lookback: int = 14) -> dict:
        """Detect RSI divergence over completed candles only (2-candle confirmation).

        Bearish divergence: price makes higher high, RSI makes lower high.
        Bullish divergence: price makes lower low, RSI makes higher low.
        """
        if len(dataframe) < lookback + 3:
            return {"bearish": False, "bullish": False}

        segment = dataframe.iloc[-(lookback + 2):-1]

        peak_mask = (
            (segment["high"] > segment["high"].shift(1)) &
            (segment["high"] >= segment["high"].shift(-1))
        )
        peak_indices = segment.index[peak_mask].tolist()

        bearish = False
        if len(peak_indices) >= 2:
            i1, i2 = peak_indices[-2], peak_indices[-1]
            if (segment.loc[i1, "high"] < segment.loc[i2, "high"] and
                segment.loc[i1, "rsi"] > segment.loc[i2, "rsi"]):
                bearish = True

        trough_mask = (
            (segment["low"] < segment["low"].shift(1)) &
            (segment["low"] <= segment["low"].shift(-1))
        )
        trough_indices = segment.index[trough_mask].tolist()

        bullish = False
        if len(trough_indices) >= 2:
            i1, i2 = trough_indices[-2], trough_indices[-1]
            if (segment.loc[i1, "low"] > segment.loc[i2, "low"] and
                segment.loc[i1, "rsi"] < segment.loc[i2, "rsi"]):
                bullish = True
    ...
    def _record_outcome(self, trade: Trade, profit_pct: float) -> None:
        try:
            ...
            record = {
                ...
                "is_win": is_win,
                "strategy": "VectorStrategy_P3A_RSI_DIVERGENCE_EXIT",
