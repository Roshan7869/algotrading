...
use crate::chart::{comparison, heatmap, kline, orderflow, volumeprofile};
...
use crate::chart::{
    ...
    indicator::{HeatmapIndicator, KlineIndicator, OrderFlowIndicator},
...
pub enum VisualConfig {
    ...
    VolumeProfile(volumeprofile::Config),
}
...
impl VisualConfig {
    ...
    pub fn ladder(&self) -> Option<ladder::Config> {
        match self {
            Self::Ladder(cfg) => Some(cfg.clone()),
            _ => None,
        }
    }
    ...
    pub fn orderflow(&self) -> Option<orderflow::Config> {
        match self {
            Self::Orderflow(cfg) => Some(*cfg),
            _ => None,
        }
    }

    pub fn volume_profile(&self) -> Option<volumeprofile::Config> {
        match self {
...
impl std::fmt::Display for ContentKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ...
            ContentKind::OrderflowChart => "Orderflow Chart",
            ContentKind::VolumeProfileChart => "Volume Profile Chart",
        };
        write!(f, "{s}")
    }
}
    ...
    pub fn new(
        ...
        let basis =
            match content_kind {
                ...
                ContentKind::OrderflowChart => {
                    Some(current_basis.unwrap_or_else(|| {
                        Basis::default_kline_time(Some(base_ticker), Timeframe::M5)
                    }))
                }
                ContentKind::VolumeProfileChart => {
        ...
        let tick_multiplier = match content_kind {
            ...
            ContentKind::OrderflowChart | ContentKind::VolumeProfileChart => {
                Some(current_tick_multiplier.unwrap_or(TickMultiplier(50)))
            }
        };
