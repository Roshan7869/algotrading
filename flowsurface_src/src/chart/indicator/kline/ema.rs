...
impl EmaIndicator {
    pub fn new() -> Self {
        Self {
            ...
            period: 9,
            color: Color::from_rgb(1.0, 0.41, 0.71),
        }
    }
    ...
    fn compute_from_source(&mut self, source: &PlotData<KlineDataPoint>) {
        match source {
            PlotData::TimeBased(ts) => {
                ...
                let ema_vals = ta::ema_series(&closes, self.period);
                let result: std::collections::BTreeMap<UnixMs, EmaPoint> = ts.datapoints.iter().zip(ema_vals.iter())
                    .filter_map(|((&t, _), opt)| opt.map(|v| (t, EmaPoint { ema: v })))
                    .collect();
            ...
            PlotData::TickBased(tick) => {
                ...
                if closes.len() < self.period { return; }
                let ema_vals = ta::ema_series(&closes, self.period);
                let result: std::collections::BTreeMap<u64, EmaPoint> = ema_vals.iter().enumerate()
                    .filter_map(|(i, opt)| opt.map(|v| (i as u64, EmaPoint { ema: v })))
                    .collect();
...
impl KlineIndicatorImpl for EmaIndicator {
    ...
    fn draw_overlay(&self, frame: &mut Frame, ctx: &ViewState, _theme: &Theme, visible_range: RangeInclusive<u64>) {
        ...
        let points: Vec<(u64, &EmaPoint)> = match &self.data {
        ...
        let path = Path::new(|builder| {
            ...
            for &(ts, pt) in &points {
                ...
                let y = ctx.price_to_y(Price::from_f32_lossy(pt.ema));
    ...
    fn element<'a>(&'a self, _chart: &'a ViewState, _data_labels_always_visible: bool, _visible_range: RangeInclusive<u64>) -> iced::Element<'a, Message> {
    ...
    fn apply_config(&mut self, config: &IndicatorConfig) {
        if let IndicatorConfig::Ema { period, color } = config {
            self.period = *period;
            if let Ok(c) = crate::chart::indicator::kline::sma::parse_hex_color(color) { self.color = c; }
        }
    }
}
