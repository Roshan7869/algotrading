...
impl KlineChart {
        ...
        match basis {
            Basis::Time(interval) => {
                ...
                for &i in enabled_indicators {
                    let mut indi = indicator::kline::make_with_config(i, &visual_config.indicator_params);
    ...
    pub fn set_indicator_config(
        &mut self,
        which: KlineIndicator,
        config: data::chart::indicator::IndicatorConfig,
    ) {
        self.visual_config.indicator_params.insert(which, config);
        if let Some(indi) = self.indicators[which].as_mut() {
            indi.apply_config(
                self.visual_config
                    .indicator_params
                    .get(&which)
                    .unwrap_or(&data::chart::indicator::IndicatorConfig::for_kline(which)),
            );
    ...
    pub fn toggle_indicator(&mut self, indicator: KlineIndicator) {
        ...
        if self.indicators[indicator].is_some() {
            ...
            let mut box_indi = indicator::kline::make_with_config(indicator, &self.visual_config.indicator_params);
        ...
        if current_indi_count > 0 && self.chart.layout.splits.len() != current_indi_count {
            self.chart.layout.splits = data::util::calc_panel_splits(
                0.7,
                current_indi_count,
                Some(prev_indi_count),
            );
        } else if let Some(main_split) = self.chart.layout.splits.first() {
            self.chart.layout.splits = data::util::calc_panel_splits(
                *main_split,
                current_indi_count,
                Some(prev_indi_count),
...
impl canvas::Program<Message> for KlineChart {
    ...
    fn draw(
        ...
        let klines = chart.cache.main.draw(renderer, bounds_size, |frame| {
            ...
            for indi in self.indicators.values().filter_map(|o| o.as_deref()) {
                indi.draw_overlay(frame, chart, theme, earliest..=latest);
            }
