...
impl From<&pane::State> for data::Pane {
    fn from(pane: &pane::State) -> Self {
        ...
        match &pane.content {
            ...
            pane::Content::Orderflow { chart, indicators, layout, .. } => {
                data::Pane::OrderflowChart {
                    layout: chart.as_ref().map_or(layout.clone(), |c| c.chart_layout()),
                    stream_type: streams,
                    settings: pane.settings.clone(),
                    indicators: indicators.clone(),
            ...
            pane::Content::VolumeProfile { chart, indicators, layout, .. } => {
                data::Pane::VolumeProfileChart {
                    layout: chart.as_ref().map_or(layout.clone(), |c| c.chart_layout()),
                    stream_type: streams,
                    settings: pane.settings.clone(),
                    indicators: indicators.clone(),
...
pub fn configuration(pane: data::Pane) -> Configuration<pane::State> {
    match pane {
        ...
        data::Pane::OrderflowChart {
            ...
            let content = pane::Content::Orderflow {
                chart: None,
                indicators,
                layout,
            };
        ...
        data::Pane::VolumeProfileChart {
            ...
            let content = pane::Content::VolumeProfile {
                chart: None,
                indicators,
                layout,
            };
