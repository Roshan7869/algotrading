...
use data::util::calc_panel_splits;
use std::sync::OnceLock;

static DEFAULT_SPLITS_CACHE: OnceLock<Vec<f32>> = OnceLock::new();
...
pub enum Message {
    ...
    IndicatorConfigChanged(data::chart::indicator::KlineIndicator, data::chart::indicator::IndicatorConfig),
}
...
pub fn update<T: Chart>(chart: &mut T, message: &Message) {
    match message {
        ...
        Message::IndicatorConfigChanged(..) => {}  // handled at pane level
    }
...
pub fn view<'a, T: Chart>(
    ...
    let content = {
        ...
        let indicator_elements = chart.view_indicators(indicators);

        if indicator_elements.is_empty() {
            main_chart
        } else {
            let panels = std::iter::once(main_chart)
                .chain(indicator_elements)
                .collect::<Vec<_>>();

            let is_active = panels.len() - 1;
            let splits: &Vec<f32> = if state.layout.splits.len() == is_active {
                &state.layout.splits
            } else {
                DEFAULT_SPLITS_CACHE.get_or_init(|| calc_panel_splits(0.7, is_active, None))
            };
