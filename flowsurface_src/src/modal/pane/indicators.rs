use crate::screen::dashboard::pane::{self, Message};
use crate::style::{self, Icon, icon_text};
use crate::widget::{column_drag, dragger_row};

use data::chart::indicator::{Indicator, KlineIndicator, UiIndicator};
use iced::{
    Element, Length, padding,
    widget::{button, column, container, pane_grid, row, space, text, pick_list},
};

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PresetLayout {
    Classic,
    Scalping,
    Swing,
    OrderFlow,
    Trend,
}

impl PresetLayout {
    pub fn all() -> &'static [PresetLayout] {
        &[Self::Classic, Self::Scalping, Self::Swing, Self::OrderFlow, Self::Trend]
    }

    pub fn indicators(&self) -> Vec<KlineIndicator> {
        match self {
            Self::Classic => vec![
                KlineIndicator::Vwap,
                KlineIndicator::BollingerBands,
            ],
            Self::Scalping => vec![
                KlineIndicator::Volume,
                KlineIndicator::PerCandleDelta,
                KlineIndicator::PerCandleImbalance,
                KlineIndicator::Rvol,
            ],
            Self::Swing => vec![
                KlineIndicator::Vwap,
                KlineIndicator::BollingerBands,
                KlineIndicator::Alma,
                KlineIndicator::Rsi,
                KlineIndicator::Macd,
                KlineIndicator::Lvn,
            ],
            Self::OrderFlow => vec![
                KlineIndicator::CumulativeDelta,
                KlineIndicator::PerCandleDelta,
                KlineIndicator::PerCandleAbsorption,
                KlineIndicator::PerCandleZScore,
                KlineIndicator::PerCandleImbalance,
            ],
            Self::Trend => vec![
                KlineIndicator::Alma,
                KlineIndicator::BollingerBands,
                KlineIndicator::Atr,
                KlineIndicator::Adx,
                KlineIndicator::Rvol,
            ],
        }
    }
}

impl std::fmt::Display for PresetLayout {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Classic => write!(f, "Classic"),
            Self::Scalping => write!(f, "Scalping"),
            Self::Swing => write!(f, "Swing"),
            Self::OrderFlow => write!(f, "Order Flow"),
            Self::Trend => write!(f, "Trend"),
        }
    }
}

pub fn view<'a, I>(
    pane: pane_grid::Pane,
    state: &'a pane::State,
    selected: &[I],
    market_type: Option<exchange::adapter::MarketKind>,
) -> Element<'a, Message>
where
    I: Indicator + Copy + Into<UiIndicator>,
{
    let content_allows_dragging = matches!(state.content, pane::Content::Kline { .. });
    let content_row = if let Some(market) = market_type {
        content_row(pane, selected, market, content_allows_dragging)
    } else {
        column![].spacing(4).into()
    };

    container(content_row)
        .max_width(200)
        .padding(16)
        .style(style::chart_modal)
        .into()
}

fn build_indicator_row<'a, I>(
    pane: pane_grid::Pane,
    indicator: &I,
    is_selected: bool,
) -> Element<'a, Message>
where
    I: Indicator + Copy + Into<UiIndicator>,
{
    let content = if is_selected {
        row![
            text(indicator.to_string()),
            space::horizontal(),
            container(icon_text(Icon::Checkmark, 12)),
            container(
                button(icon_text(Icon::Cog, 12))
                    .on_press(Message::PaneEvent(
                        pane,
                        pane::Event::IndicatorConfigToggled((*indicator).into()),
                    ))
                    .style(iced::widget::button::text)
            ),
        ]
        .width(Length::Fill)
    } else {
        row![text(indicator.to_string())].width(Length::Fill)
    };

    button(content)
        .on_press(Message::PaneEvent(
            pane,
            pane::Event::ToggleIndicator((*indicator).into()),
        ))
        .width(Length::Fill)
        .style(move |theme, status| style::button::modifier(theme, status, is_selected))
        .into()
}

fn selected_list<'a, I>(
    pane: pane_grid::Pane,
    selected: &[I],
    reorderable: bool,
) -> Element<'a, Message>
where
    I: Indicator + Copy + Into<UiIndicator>,
{
    let elements: Vec<Element<_>> = selected
        .iter()
        .map(|indicator| {
            let base = build_indicator_row(pane, indicator, true);
            dragger_row(base, reorderable)
        })
        .collect();

    if reorderable {
        let mut draggable_column = column_drag::Column::new()
            .on_drag(move |event| Message::PaneEvent(pane, pane::Event::ReorderIndicator(event)))
            .spacing(4);
        for element in elements {
            draggable_column = draggable_column.push(element);
        }
        draggable_column.into()
    } else {
        iced::widget::Column::with_children(elements)
            .spacing(4)
            .into()
    }
}

fn available_list<'a, I>(pane: pane_grid::Pane, available: &[I]) -> Element<'a, Message>
where
    I: Indicator + Copy + Into<UiIndicator>,
{
    let elements: Vec<Element<_>> = available
        .iter()
        .map(|indicator| {
            let base = build_indicator_row(pane, indicator, false);
            dragger_row(base, false)
        })
        .collect();

    iced::widget::Column::with_children(elements)
        .spacing(4)
        .into()
}

fn content_row<'a, I>(
    pane: pane_grid::Pane,
    selected: &[I],
    market: exchange::adapter::MarketKind,
    allows_drag: bool,
) -> Element<'a, Message>
where
    I: Indicator + Copy + Into<UiIndicator>,
{
    let reorderable = allows_drag && selected.len() >= 2;

    let selected_list = if !selected.is_empty() {
        Some(selected_list(pane, selected, reorderable))
    } else {
        None
    };

    let available: Vec<I> = I::for_market(market)
        .iter()
        .filter(|indicator| !selected.contains(indicator))
        .cloned()
        .collect();
    let available_list = if !available.is_empty() {
        Some(available_list(pane, &available))
    } else {
        None
    };

    let mut col = iced::widget::Column::new();
    if let Some(sel) = selected_list {
        col = col.push(sel);
    }
    if let Some(avail) = available_list {
        col = col.push(avail);
    }

    let preset_pick = pick_list(
        PresetLayout::all(),
        None::<PresetLayout>,
        move |preset| {
            Message::PaneEvent(pane, pane::Event::SetIndicators(preset.indicators()))
        },
    )
    .placeholder("Load preset…")
    .padding(4)
    .text_size(11);

    column![
        container(text("Indicators").size(crate::style::text_size::SECTION))
            .padding(padding::bottom(8)),
        container(preset_pick).padding(padding::bottom(4)),
        col.spacing(4)
    ]
    .spacing(4)
    .into()
}
